select
	...
	IND.TVA RFC,
    Sum(tabla.Rentas_Devengar) * (1 + Max(tabla.TASA_IVA)) Rentas_Devengar,
    Sum(tabla.RENTAS_DEVENGAR + ta.RESIDUAL_VALUE)  * (1 + Max(tabla.TASA_IVA)) Rentas_Devengar_Residual,
    Sum(tabla.SALDO_INS_CAPITAL + ta.RESIDUAL_VALUE)  * (1 + Max(tabla.TASA_IVA)) Riesgo_Expuesto,
    NVL(Sum(OCon.Otros_Conceptos) * (1 + Max(tabla.TASA_IVA)), 0) Otros_Conceptos_Vencidos,
    NVL(Sum(Venc.Rentas_Vencidas) * (1 + Max(tabla.TASA_IVA)), 0) Total_Exigible
	from imxdb.G_DOSSIER DOSS
		...
		inner join (
			...
			from imxdb.T_FIN_AMO c
            inner join (Select IMX_UN_ID, TYPE, STR3, DT04_DT From IMXDB.g_piecedet Where TYPE = 'ANNEXE_DEMANDE_FIN' AND STR3 = 'ACT')det on det.imx_un_id = c.ANNEX_ID
	...
	from (
		select amort.fin_amort_id, amort.capital_installment Capital, amort.interest_installment interes, nvl(ent.ER_DAT_DT,amort.instal_due_dt) FEC_FACTURADO
